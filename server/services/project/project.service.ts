...
export async function fetchProjectById(projectId: string, userId: string) {
  ...
  const res = await db
    .select({
      ...
      contributors:
        sql`json_agg(DISTINCT jsonb_build_object('id', ${user.id}, 'name', ${user.name}, 'email', ${user.email}, 'role', ${user.role}, 'avatarUrl', ${user.avatarUrl}))`.as(
          'contributors'
        ),
      detectors:
    ...
    .leftJoin(user, eq(projectContributor.contributorId, user.id))
    .leftJoin(detector, eq(projectDetector.detectorId, detector.id))
